...
func init() {
	logUS = kitlog.CategoryLogger(kitlog.SRV)
...
func (p *pinger) recvICMP(pTimeout time.Duration, pConn *icmp.PacketConn, pRandomID int, pChanRecv chan<- *Packet) {
	vWait := time.Now()
	logUS.Debug("{} | Waiting packet ({}) from {} since '{}'", p.ID, pRandomID, p.ipFrom.String(), vWait.Format("2006-01-02 15:04:05.000"))
	...
	for {
		...
		if vGeneralTimeout < pTimeout {
			...
			logUS.Info("{} | General Timeout. No suitable packet recieved within {}", p.ID, vGeneralTimeout)
		...
		if err != nil {
			if neterr, ok := err.(*net.OpError); ok {
				...
				logUS.Warn("{} | Unexpected Error. New attempt...: {}", p.ID, neterr)
			...
			if err != nil {
				logUS.Error("{} | Unexpected error processing the packet: {}", p.ID, err)
				...
				if processedPacket == nil {
					logUS.Warn("{} | Packet recieved is not an ICMP one", p.ID)
					vNRecievedPacket++
				} else {
					if processedPacket.ID == p.ID && processedPacket.PacketID == pRandomID {
						vReply := time.Now()
						logUS.Info("{} | Reply packet ({}) recieved within {} from {} at '{}'", p.ID, strconv.Itoa(processedPacket.PacketID), processedPacket.Time, p.ipFrom.String(), vReply.Format("2006-01-02 15:04:05.000"))
						pChanRecv <- processedPacket
						break
					} else {
						logUS.Warn("{} | Huh ? Unexpected reply recieved with ID ({}) and packet ID ({}). Expected packet ID:{}", p.ID, processedPacket.ID, strconv.Itoa(processedPacket.PacketID), pRandomID)
						vNRecievedPacket++
					}
				}
			}
...
func (p *pinger) sendICMP(conn *icmp.PacketConn, pRandomID int) error {
	vSendDate := time.Now()
	logUS.Info("{} | Sending packet ({}) to {} since '{}'", p.ID, pRandomID, p.ipAddrTo.String(), vSendDate.Format("2006-01-02 15:04:05.000"))
	var typ icmp.Type
	if p.ipv4 {
...
func (p *pinger) listen(netProto string, source string) *icmp.PacketConn {
	...
	if err != nil {
		logUS.Error("{} | Error listening for ICMP packets: {}", p.ID, err.Error())
		return nil
	}
	return conn
}
